...
# ---------------------------------------------------------------------------
# GABC helpers (shared with fix_missing_scores.py logic)
# ---------------------------------------------------------------------------

def extract_gabc_body(gabc_raw: str) -> str:
    if not gabc_raw:
        return ''
    try:
        obj = json.loads(gabc_raw)
        ...
        pass
    parts = gabc_raw.split('%%')
    return parts[-1].strip() if len(parts) > 1 else gabc_raw.strip()
